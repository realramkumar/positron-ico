...
const contractAddress = '0xEf4ad2dadA8F74f1d37a2Fba012d90f04368396c';

/// getting contract
const web3 = new Web3(new Web3.providers.HttpProvider(targetApi));
const positronTokenIco = web3.eth.contract(abi).at(contractAddress);

/// read and display values
writeValue('contractAddress', contractAddress);
writeValue('totalRaised', toEthString(positronTokenIco.totalRaised()));

const startDate = hexSecondsToMillis(positronTokenIco.startTimestamp());
const endDate = startDate + hexSecondsToMillis(positronTokenIco.durationSeconds());
writeValue('startDate', new Date(startDate).toLocaleString());
writeValue('endDate', new Date(endDate).toLocaleString());

writeValue('minCap', toEthString(positronTokenIco.minCap()));
writeValue('maxCap', toEthString(positronTokenIco.maxCap()));
